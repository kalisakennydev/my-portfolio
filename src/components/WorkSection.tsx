...
const projects = [
  {
    ...
    live: "https://github.com/kalisakennydev",
    code: "https://github.com/kalisakennydev",
  },
  {
    num: "02",
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
        {projects.map((p, i) => (
          <motion.div
            ...
            className="group bg-card rounded-2xl overflow-hidden border shadow-sm hover:shadow-2xl hover:-translate-y-3 transition-all duration-400"
          >
            <div className="relative overflow-hidden">
              <img src={p.img} alt={p.title} className="w-full h-48 sm:h-52 object-cover group-hover:scale-110 transition-transform duration-500" />
            </div>
            <div className="p-5 sm:p-6">
              ...
              <div className="flex gap-4 text-sm font-medium">
                <a href={p.live} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-primary hover:underline"><ExternalLink size={14} /> View Live</a>
                <a href={p.code} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-muted-foreground hover:text-foreground"><Code2 size={14} /> Code</a>
              </div>
            </div>
          </motion.div>
        ))}
